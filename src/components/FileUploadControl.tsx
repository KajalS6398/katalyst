...
const defaultGetFileIcon = (fileName: string, fileType: string) => {
  ...
  if (
    ...
    return <HiPhotograph className="w-5 h-5 text-white" />;
  }
  ...
  if (
    ...
    return <HiOutlineMusicNote className="w-5 h-5 text-white" />;
  }
  ...
  if (
    ...
    return <HiOutlineVideoCamera className="w-5 h-5 text-white" />;
  }
  ...
  if (
    ...
    return <HiOutlineDocumentAdd className="w-5 h-5 text-white" />;
  }
  ...
  if (
    ...
    return <HiOutlineDocumentReport className="w-5 h-5 text-white" />;
  }

  // POWERPOINT FILES
  if (["pptx", "pptm", "xps", "ppsx"].includes(extension)) {
    return <HiOutlineViewBoards className="w-5 h-5 text-white" />;
  }
  ...
  if (
    ...
    return <HiOutlineCollection className="w-5 h-5 text-white" />;
  }

  // PDF FILES
  if (fileType === "application/pdf" || extension === "pdf") {
    return <HiOutlineFolderOpen className="w-5 h-5 text-white" />;
  }

  // DEFAULT ICON
  return <HiOutlineDocumentText className="w-5 h-5 text-white" />;
};
...
export default function FileUploadControl({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLLabelElement>) => {
    if (e.key === "Enter" || e.key === " ") {
      e.preventDefault();
      // Only trigger if the event target is the label itself
